...
export default {
    ...
    setDataset(dom){
        ...
        // debugger
        // console.log(aa.attr())
        // var attrs = dom.getAttributeNames();
        // var datasetKeys = attrs.filter(it=>/^data\-/.test(it))
        var dataset = {};
        var len = dom.attributes.length;
        for(var i=0;i<len;i++){
            let it = dom.attributes[i];
            let localName = it.localName;
            if(/^data\-/.test(localName)){
                let key = localName.replace('data-','');
                dataset[key] = it.value;
            }
        }
        // alert(dom.attributes[0])
        
        // datasetKeys.map(it=>{
        //     var key = it.replace('data-','');
        //     dataset[key] = dom.getAttribute(it);
        // })
        return dataset;
    }
};
